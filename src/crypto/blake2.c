...
CFX_STATIC_ASSERT(sizeof(cfx_blake2b_state_t) <= sizeof(cfx_blake2b_ctx_t),
                  blake2b_ctx_too_small);
CFX_STATIC_ASSERT(sizeof(cfx_blake2s_state_t) <= sizeof(cfx_blake2s_ctx_t),
                  blake2s_ctx_too_small);
...
#define B2B_G_AVX2(row0, row1, row2, row3, m_x, m_y)                           \
  do {                                                                         \
...
#define DIAG_PERM(row1, row2, row3)                                            \
  do {                                                                         \
...
#define UNDIAG_PERM(row1, row2, row3)                                          \
  do {                                                                         \
...
static void blake2b_compress_avx2(cfx_blake2b_state_t *S,
  ...
  row3 = _mm256_xor_si256(
      row3, _mm256_setr_epi64x((int64_t)S->t[0], (int64_t)S->t[1],
                               (int64_t)S->f[0], (int64_t)S->f[1]));

  for (int r = 0; r < 12; r++) {
    const uint8_t *s = sigma[r];

    /* column round */
    __m256i mx0 = _mm256_setr_epi64x((int64_t)load64_le(block + s[0] * 8),
                                     (int64_t)load64_le(block + s[2] * 8),
                                     (int64_t)load64_le(block + s[4] * 8),
                                     (int64_t)load64_le(block + s[6] * 8));
    __m256i my0 = _mm256_setr_epi64x((int64_t)load64_le(block + s[1] * 8),
                                     (int64_t)load64_le(block + s[3] * 8),
                                     (int64_t)load64_le(block + s[5] * 8),
                                     (int64_t)load64_le(block + s[7] * 8));
    B2B_G_AVX2(row0, row1, row2, row3, mx0, my0);

    DIAG_PERM(row1, row2, row3);

    /* diagonal round */
    __m256i mx1 = _mm256_setr_epi64x((int64_t)load64_le(block + s[8] * 8),
                                     (int64_t)load64_le(block + s[10] * 8),
                                     (int64_t)load64_le(block + s[12] * 8),
                                     (int64_t)load64_le(block + s[14] * 8));
    __m256i my1 = _mm256_setr_epi64x((int64_t)load64_le(block + s[9] * 8),
                                     (int64_t)load64_le(block + s[11] * 8),
                                     (int64_t)load64_le(block + s[13] * 8),
                                     (int64_t)load64_le(block + s[15] * 8));
...
int cfx_blake2b_init(cfx_blake2b_ctx_t *ctx, size_t outlen) {
  if (outlen == 0 || outlen > CFX_BLAKE2B_OUTBYTES)
    return -1;
...
int cfx_blake2b_init_key(cfx_blake2b_ctx_t *ctx, size_t outlen, const void *key,
                         size_t keylen) {
  if (outlen == 0 || outlen > CFX_BLAKE2B_OUTBYTES)
    return -1;
  if (keylen > CFX_BLAKE2B_KEYBYTES)
    return -1;
...
int cfx_blake2b(void *out, size_t outlen, const void *in, size_t inlen,
...
int cfx_blake2s_init(cfx_blake2s_ctx_t *ctx, size_t outlen) {
  if (outlen == 0 || outlen > CFX_BLAKE2S_OUTBYTES)
    return -1;
...
int cfx_blake2s_init_key(cfx_blake2s_ctx_t *ctx, size_t outlen, const void *key,
                         size_t keylen) {
  if (outlen == 0 || outlen > CFX_BLAKE2S_OUTBYTES)
    return -1;
  if (keylen > CFX_BLAKE2S_KEYBYTES)
    return -1;
...
int cfx_blake2s(void *out, size_t outlen, const void *in, size_t inlen,
                const void *key, size_t keylen) {
