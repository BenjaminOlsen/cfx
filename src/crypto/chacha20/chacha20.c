...
/*
 * Public API function - delegates to target-specific backend implementation.
 * The backend is selected at compile time via cfx_find_backend_source().
 */
void cfx_chacha20_block8_avx2(const uint8_t key[32], uint32_t counter,
                              const uint8_t nonce[12], uint8_t out[8][64]) {
    cfx_chacha20_block8_impl(key, counter, nonce, out);
}
