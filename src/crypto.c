...
/* sum: */
#define S(a,b,c,d,e) ((a)+(b)+(c)+(d)+(e))
/* cast to accumulator, multiply */
#define X(a,b) ((uint64_t)(a)*(uint64_t)(b))


/* ---------------------------------------------------------------------------------------------------- */
/* The amazing poly1305 authenticator algorithm - shamelessly plagiarized from libsodium but copied here
 ...
 * tag: 16 byte MAC
 */
void cfx_poly1305_auth(const uint8_t key[32], const uint8_t *m, size_t mlen, uint8_t tag[16]) {
    ...
    while (bytes >= 16) { /* full 16-byte blocks (each with hibit set) */
        ...
        d0 = S(X(h0,r0), X(h1,s4), X(h2,s3), X(h3,s2), X(h4,s1));
        d1 = S(X(h0,r1), X(h1,r0), X(h2,s4), X(h3,s3), X(h4,s2));
        d2 = S(X(h0,r2), X(h1,r1), X(h2,r0), X(h3,s4), X(h4,s3));
        d3 = S(X(h0,r3), X(h1,r2), X(h2,r1), X(h3,r0), X(h4,s4));
        d4 = S(X(h0,r4), X(h1,r3), X(h2,r2), X(h3,r1), X(h4,r0));
