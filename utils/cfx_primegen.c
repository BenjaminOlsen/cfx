...
static void usage(const char* prog) {
    fprintf(stderr,
        "Usage: %s <bits> [options]\n"
        "  Generate an N-bit prime.\n\n"
        "Options:\n"
        "  --safe          Generate safe (Sophie Germain) prime\n"
        "  --top2          Ensure top 2 bits are set\n"
...
static void print_big_result(const cfx_big_t* b, enum output_format fmt) {
    if (fmt == FMT_BASE64) {
        ...
        if (bytes) {
            cfx_big_to_bytes_be(bytes, &nbytes, b);
            size_t b64_len = cfx_base64_enc_len(nbytes);
            char* b64 = (char*)malloc(b64_len + 1);
            if (b64) {
                cfx_base64_encode(b64, &b64_len, bytes, nbytes);
                b64[b64_len] = '\0';
                printf("%s\n", b64);
                free(b64);
            }
