...
/* Poly1271 Encrypt benchmark */
static void BM_AEAD1271_Encrypt(benchmark::State& state, size_t aad_len) {
    const size_t pt_len = static_cast<size_t>(state.range(0));
    ...
    cfx_rand_bytes(nonce.data(), nonce.size());
    cfx_rand_bytes(pt.data(),    pt.size());
    if (aad_len) cfx_rand_bytes(aad.data(), aad.size());

    const uint8_t* aad_ptr = aad_len ? aad.data() : nullptr;

    int rc = cfx_chacha20_poly1271_encrypt(ct.data(), tag, pt.data(), pt_len, aad_ptr, aad_len, key.data(), nonce.data());
    CFX_ASSERT(rc == 0);
    rc = cfx_chacha20_poly1271_decrypt(pt_out.data(), ct.data(), pt_len, aad_ptr, aad_len, key.data(), nonce.data(), tag);
    CFX_ASSERT(rc == 0);
    CFX_ASSERT(std::memcmp(pt_out.data(), pt.data(), pt_len) == 0);
    ...
    for (auto _ : state) {
        unsigned aux;
        uint64_t start_cycles = __rdtscp(&aux);
        rc = cfx_chacha20_poly1271_encrypt(ct.data(), tag, pt.data(), pt_len, aad_ptr, aad_len, key.data(), nonce.data());
        uint64_t end_cycles = __rdtscp(&aux);
        CFX_ASSERT(rc == 0);
        tsc_ticks += (end_cycles - start_cycles);
        benchmark::DoNotOptimize(ct.data());
    ...
    state.SetBytesProcessed(static_cast<int64_t>(bytes));
    state.SetItemsProcessed(static_cast<int64_t>(iters));
    state.counters["tsc_ticks_per_iter"] = static_cast<double>(tsc_ticks) / iters;
    state.counters["tsc_ticks_per_byte"] = static_cast<double>(tsc_ticks) / bytes;
}

/* Poly1271 Decrypt benchmark */
static void BM_AEAD1271_Decrypt(benchmark::State& state, size_t aad_len) {
    const size_t pt_len = static_cast<size_t>(state.range(0));
...
int main(int argc, char** argv) {
    ...
    /* Poly1271: Encrypt, no AAD */
    {
        auto* b = benchmark::RegisterBenchmark(
            "AEAD1271_Encrypt_NoAAD",
            [](benchmark::State& state) {
                BM_AEAD1271_Encrypt(state, /*aad_len=*/0);
            });
        BENCH_ARGS(b);
    }

    /* Poly1271: Decrypt, no AAD */
    {
        auto* b = benchmark::RegisterBenchmark(
            "AEAD1271_Decrypt_NoAAD",
            [](benchmark::State& state) {
                BM_AEAD1271_Decrypt(state, /*aad_len=*/0);
            });
        BENCH_ARGS(b);
    }

    /* Poly1271: Encrypt, 32-byte AAD */
    {
        auto* b = benchmark::RegisterBenchmark(
            "AEAD1271_Encrypt_AAD32",
            [](benchmark::State& state) {
                BM_AEAD1271_Encrypt(state, /*aad_len=*/32);
            });
        BENCH_ARGS(b);
    }

    /* Poly1271: Decrypt, 32-byte AAD */
    {
        auto* b = benchmark::RegisterBenchmark(
            "AEAD1271_Decrypt_AAD32",
            [](benchmark::State& state) {
                BM_AEAD1271_Decrypt(state, /*aad_len=*/32);
            });
        BENCH_ARGS(b);
    }
